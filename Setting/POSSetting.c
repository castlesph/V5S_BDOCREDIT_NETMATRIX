...
//wifi-mod
						...
						if(BolDetachLANChange==TRUE)
						{
								...
								//chage TMS comms
								strTCT.inTMSComMode = srTransRec.usTerminalCommunicationMode;
								inTCTSave(1);

								inERMCommSet("inCommMode", srTransRec.usTerminalCommunicationMode);
...
//	vdDebug_LogPrintf("JEFF::Comm Count [%d]", inCPTNumRecs);
	...
	inERMCommSet("inCommMode", inCommMode);
...
int inSelectTelcoSetting()
{
	...
	if (key > 0)
	{
		...
		{
		    //remove-version16
			//if(i == 108 || i == 109 || i == 110 || i == 112 || i == 113 || i == 114 || i == 117)
			//	continue;
			...
			//version16
            if (i == 117){
				vdDebug_LogPrintf("test save phqr table");
				strcpy(strPHQR.szGetStatusPriIP,strGPT.szGPTPriTxnHostIP);
				strcpy(strPHQR.szGetStatusSecIP,strGPT.szGPTSecTxnHostIP);
				inPHQRSave(1);
			...
			//version16
			if(i == 108 || i == 109 || i == 110 || i == 112 || i == 113 || i == 114 || i == 117 || i == 118 || i == 111){
				vdDebug_LogPrintf("do not save port  for wallet");
			}else{ 
...
void vdCTOS_FunctionKey(void)
{
	...
	if(ret == d_KBD_ENTER)
	{
		...
		switch(atoi(strOut))
		{
			...
			//case 15:
			//	vdReUploadERMReceipt();
			//	break;
			
		    case 15:
				inCTOS_ADC_ERM_DeInit();
				break;
				
	  		case 21:
		  	memcpy(szFuncTitleName,"REPRINT CTMS REPORT",19);
		  	//if(inFunctionKeyPasswordEx("REPRINT CTMS REPORT", 2 /*SUPER_PW*/) == d_OK) //SYSTEM PASSWORD	  
		  	
			vdCTOS_SetTransType(CTMS_REPRINT);	
			//display title
...
void vdCTOS_ChangeMenuID()
{
	...
	while(1)
	{
		...
		CTOS_LCDTPrintXY(1, 7, "PLEASE ENTER [1..109]");
...
void vdCTOS_TMSSetting(void)
{
	...
	if(GPRS_MODE == strTCT.inTMSComMode){
			...
			strcpy(stgprs.strID, strTCP.szUserName);
			strcpy(stgprs.strPW, strTCP.szPassword);
			stgprs.bSIMSlot=1;// always set to 1
	        CTOS_CTMSSetConfig(d_CTMS_GPRS_CONFIG, &stgprs);
...
/*values for inIndicator = 1=check if batch has record; 2=if there's pending reversal*/
	...
	vdDebug_LogPrintf("szHostMenu2 %s %d %s", szHostMenu, strlen(szHostMenu), szHeaderString);
	...
	vdDebug_LogPrintf("inCTOS_SelectHostSettingWithIndicator key %d", key);
...
{
    vdDebug_LogPrintf("crash testing vdCTOS_uiIDLEWakeUpSleepMode");
	...
	if (inSleepflag == 1)
	{
		...
		vdDebug_LogPrintf("crash testing vdCTOS_uiIDLEWakeUpSleepMode before call vdSetIdleEvent");
		vdSetIdleEvent(1);
	}
	return;
}
...
int inCTOS_vdSetDCCMode(void)
{
	...
	CTOS_LCDTClearDisplay();
	vdDispTitleString("DCC MODE"); 
	
	vdCTOS_DispStatusMessage("PROCESSING...");
	
	inBatchCount = inDCCMMTNumRecordwithBatch();
	if(inBatchCount>0)
	{
		vduiWarningSound();
        if(strTCT.byTerminalType%2 == 0)
        {
			clearLine(14);
            vduiDisplayStringCenter(8,"BATCH NOT");
			vduiDisplayStringCenter(9,"EMPTY");	
        }
		else
		{
			clearLine(8);
			vduiDisplayStringCenter(4,"BATCH NOT");
			vduiDisplayStringCenter(5,"EMPTY");	
		}
	...
	inTCTRead(1);
	//setLCDPrint(2, DISPLAY_POSITION_LEFT, "DCC MODE:");
	switch(strTCT.inDCCMode)
	{
		case 1:
			setLCDPrint(2, DISPLAY_POSITION_LEFT, "NORMAL");
			break;
		case 2:
			setLCDPrint(2, DISPLAY_POSITION_LEFT, "AUTO OPT IN/OUT ");
			break;
		case 3:
			setLCDPrint(2, DISPLAY_POSITION_LEFT, "MANUAL OPT OUT");
			break;
		//wifi-mod2 	
		case 4:
			setLCDPrint(2, DISPLAY_POSITION_LEFT, "PRINT DCC RATES");
			break;
		//wifi-mod2
...
//	CTOS_LCDTPrintXY(1, 3, "1:NORMAL	 2:AUTO OPT");
//	CTOS_LCDTPrintXY(1, 4, "3:MAN-OPT  4:PRINT DCC");

	CTOS_LCDTPrintXY(1, 3, "1 - NORMAL");
	CTOS_LCDTPrintXY(1, 4, "2 - AUTO OPT IN/OUT");	 
	CTOS_LCDTPrintXY(1, 5, "3 - MANUAL OPT OUT");	 
	CTOS_LCDTPrintXY(1, 6, "4 - PRINT DCC RATES");	 
	CTOS_LCDTPrintXY(1, 7, "New:");
	...
	do
	{
		...
		switch(ret)
		{
			...
			case 1:
				...
				{
					case 0x31:
						inDCCMode = NORMAL_MODE;
						break;
					case 0x32:
						inDCCMode = AUTO_OPTOUT_MODE;
						break;
					case 0x33:
						inDCCMode = MANUAL_OPTOUT_MODE;
						break;
//wifi-mod2
					case 0x34:
						inDCCMode = PRINT_DCC_RATES;
						break;
//wifi-mod2
	...
	vdDebug_LogPrintf("inDCCMode [%d]", inDCCMode);
...
//	vdDebug_LogPrintf("JEFF::Comm Count [%d]", inCPTNumRecs);
	...
	vduiClearBelow(2);
	CTOS_LCDTPrintXY(1, 8, "Please wait...");
	strTCT.inDCCMode = inDCCMode;
	inTCTSave(1);
	inTCTRead(1);
	vdDebug_LogPrintf("inDCCMode [%d]", strTCT.inDCCMode);
	/*BDO: Update fDHCPEnable for GPRS/WIFI*/
