...
//gcitra-get card holder number
    ...
    DebugAddHEX("[CREDIT1347] vdCTOSS_WaveGetEMVData T9F6E", srTransRec.stEMVinfo.T9F6E, srTransRec.stEMVinfo.T9F6E_len);
	vdDebug_LogPrintf("srTransRec.stEMVinfo.T9F6E_len[%d]",srTransRec.stEMVinfo.T9F6E_len);


	//missing 9F6E VALUE on JCB CTLS installment transacation. #1
	#if 1	
	if(srTransRec.stEMVinfo.T9F6E_len > 0){

		char szJCB9F6E[3+1];
		memset(szJCB9F6E,0x00,sizeof(szJCB9F6E));

		wub_hex_2_str(srTransRec.stEMVinfo.T9F6E, szJCB9F6E, sizeof(szJCB9F6E));
		put_env("JCB9F6E", szJCB9F6E, sizeof(szJCB9F6E));
	}
	#endif	
	//missing 9F6E VALUE on JCB CTLS installment transacation. #1

	usCTOSS_FindTagFromDataPackage(TAG_9F7C_CED, srTransRec.stEMVinfo.T9F7C, &usLen, baChipData, usChipDataLen);
	DebugAddHEX("T9F7C", srTransRec.stEMVinfo.T9F7C, usLen);
	vdDebug_LogPrintf("T9F7C,len[%d]",usLen);

	DebugAddHEX("baChipData", baChipData, usChipDataLen);
	

	
	vdDebug_LogPrintf("--vdCTOSS_WaveGetEMVData-END-");
}
...
*/
	...
	DebugAddHEX("usCTOSS_CtlsV3Trans BASE AMOUNT",srTransRec.szBaseAmount,AMT_BCD_SIZE);
