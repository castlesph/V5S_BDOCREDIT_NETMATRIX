...
int inCTOS_InitComm(int inCommType)
{
	...
	vdDebug_LogPrintf("inCTOS_InitComm|inCommType[%d]", inCommType);
	vdDebug_LogPrintf("inCTOS_InitComm|fShareComEnable[%d]", strTCT.fShareComEnable);
	vdDebug_LogPrintf("inCTOS_InitComm|byTerminalType[%d]", strTCT.byTerminalType);
	...
	{   
...
#endif
		{
			...
			{
			    vdDebug_LogPrintf("initcomm test share com enable");
				srCommFuncPoint.inInitComm = inCTOSS_COMM_Initialize;
				srCommFuncPoint.inGetCommConfig = inGPRS_GetConfigFromTable;
				srCommFuncPoint.inSetCommConfig = inGPRS_SetConnectConfig;
				srCommFuncPoint.inCheckComm = inGPRS_CheckComm;
...
#endif
    vdDebug_LogPrintf("**inModem_Disconnection START**");
    shRet = CTOS_ModemHookOn();
    
    vdDebug_LogPrintf(". CTOS_ModemHookOn(%04x)",shRet);
    vdDebug_LogPrintf("**inModem_Disconnection END**");
    ...
    while(1)
    {
        if (CTOS_ModemTxReady()== d_OK){
			...
			if ((strTCT.byTerminalType%2) == 0)
			{
       		...
       		if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
      			vdDispTransTitleCardTypeandTotal(srTransRec.byTransType); //aaronnino for BDOCLG ver 9.0 fix on issue #00139 HAVE A DEFAULT TITLE DISPLAY OF TXN TYPE CARD LABEL 7 of  8
...
#else
	if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
	    vdDispTransTitleCardTypeandTotal(srTransRec.byTransType); //aaronnino for BDOCLG ver 9.0 fix on issue #00139 HAVE A DEFAULT TITLE DISPLAY OF TXN TYPE CARD LABEL 8 of 8
...
void vdCTOS_DispStatusMessage(char* szMsg)
{
    if ((strTCT.byTerminalType%2) == 0)
    {
        //clearLine(14);// this function is slow
        CTOS_LCDTPrintXY(1, 14, "                                        ");
        CTOS_LCDTPrintXY(1, 14, szMsg);
    }
    else
    {
        //clearLine(8);// this function is slow
...
/*   
    ...
    return d_OK;
*/
    if(memcmp(szTempModemStatusRet, "0000080E", 8)==0)
		 return d_OK;
	else
		return d_NO;
...
//version16
int inEthernet_GetTerminalMac(char *szTerminalMac)
{
	BYTE bLength;
	int result;
	BYTE tmpbabuff[d_BUFF_SIZE];
	...
	memset(tmpbabuff,0x00,sizeof(tmpbabuff));
	bLength = sizeof(tmpbabuff);
	result = CTOS_EthernetConfigGet(d_ETHERNET_CONFIG_MAC, tmpbabuff, &bLength);
    vdDebug_LogPrintf("testlang getmac =[%d][%s]",result,tmpbabuff);
    strcpy(szTerminalMac, tmpbabuff);
	return ST_SUCCESS;
